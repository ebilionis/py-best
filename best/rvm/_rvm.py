"""Relevance Vector Machines Class
...
import math
from .. import Object
from ..linalg import GeneralizedSVD
from ..maps import GeneralizedLinearModel
from ..maps import CovarianceFunctionBasis
...
class RelevanceVectorMachine(Object):
    ...
    def _fix_relevant_gsvd(self):
        ...
        # Compute the gsvd
        self._gsvd = GeneralizedSVD(A, B) # Do not do V
    ...
    def __init__(self, name='Relevance Vector Machine'):
        """Initialize the object.

        Keyword Arguments:
            name    ---     A name for the object.
        """
        super(RelevanceVectorMachine, self).__init__(name=name)
    ...
    def get_generalized_linear_model(self, basis):
        ...
        """
        if isinstance(basis, CovarianceFunctionBasis):
            sp_basis = CovarianceFunctionBasis(basis.cov,
                                               basis.X[self.relevant, :],
                                               hyp=basis.hyp)
        else:
            sp_basis = basis.screen(out_idx=self.relevant)
        return GeneralizedLinearModel(sp_basis, weights=self.weights,
                                      sigma_sqrt=self.sigma_sqrt,
                                      beta=self.beta)

    def _to_string(self, pad):
        """Return a string representation of the object."""
        s = super(RelevanceVectorMachine, self)._to_string(pad) + '\n'
        s += pad + ' Y shape: ' + str(self.Y.shape) + '\n'
        s += pad + ' PHI shape: ' + str(self.PHI.shape) + '\n'
        s += pad + ' Relevant: ' + str(self.relevant) + '\n'
