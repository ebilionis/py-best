...
class SPGPSMC(object):
    ...
    def log_like(self):
        ...
        log_like = np.array([p['log_like'] for p in self.particles])
        return log_like
        ...
        if self.use_mpi:
            ...
            for i in xrange(self.num_particles):
                ...
                if from_whom == to_whom and to_whom == self.rank:
                    ...
                    self._particles.append(deepcopy(old_particles[my_idx]))
            ...
            self._particles = [deepcopy(self._particles[i]) for i in idx]
    ...
    def __init__(self, mcmc_sampler=None,
                 ...
                 lowest_acceptance_rate=0.5,
                 highest_acceptance_rate=0.8,
        ...
        if mcmc_sampler is not None:
            ...
            raise ValueError('Specify an mcmc_sampler or ')
    ...
    def sample(self):
        ...
        for i in range(self.my_num_particles):
            self._particles.append(self.mcmc_sampler.target.prior.sample())
        for p in self.particles:
            self.mcmc_sampler.target.d_eval(p)
        ...
        #for p in self.particles:
        #    plt.plot(np.exp(-p['log_b']/2)[0], math.exp(p['log_c']/2), 'r.')
        #plt.pause(0.01)
        ...
        while self.mcmc_sampler.target.gamma < 1.:
            ...
            for p in self.particles:
                self.mcmc_sampler.target.update(p)
            ...
            for p in self.particles:
                self.mcmc_sampler.sample(p, steps=self.num_mcmc)
            ...
            #    plt.plot(np.exp(-p['log_b']/2)[0], math.exp(p['log_c']/2), 'bo',
            #             markersize=100.*math.exp(log_w))
            #plt.axis([0, 5, 0, 5])
            #plt.pause(0.01)
        r = self.particles
        if self.use_mpi:
            r = self.comm.allgather(r)
